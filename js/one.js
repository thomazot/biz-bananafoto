...
$j.fn.neonTheme.custom = {
    ...
    addSVG: {
        ...
        'z-discont': {
            selector: '.header-container .mymenu .offers a',
            mode: 'prepend',
        },
        'z-menu': {
...
function showcaseTabs($) {
    var showcase = $('.showcase-tabs')

    // Carousel

    $('.showcase-tabs__header').owlCarousel({
        itemsCustom: [
            [0, 2],
            [568, 3],
            [768, 5],
    ...
    showcase.each(function() {
        var box = $(this)
        var headerLink = box.find('.showcase-tabs__header a')
        headerLink.click(function(event) {
            ...
            var id =
                ...
                $(this)
                    .attr('href')
                    .replace('#', '')
            box.find('.showcase-tabs__header a').removeClass('actived')
            $(this).addClass('actived')
            box.find('[class*=tab]').removeClass('actived')
            box.find(id).addClass('actived')
        })

        checkCarousel(box.find('.showcase-tabs__header'))
    })
}

function checkCarousel(box) {
    var controls = box.find('.owl-controls')

    if ($j(controls).attr('style')) {
        if (
            $j(controls)
                .attr('style')
                .indexOf('display: none') !== -1
        ) {
            box.removeClass('carousel-actived')
        } else box.addClass('carousel-actived')
    } else box.addClass('carousel-actived')
}
...
$j(document)
    .ready(function($) {
        ...
        $('.category-description .actions a').click(function(event) {
            event.preventDefault()

            if (
                $(this)
                    .closest('.category-description')
                    .find('.hide').length
            )
                $(this)
                    .closest('.category-description')
        ...
        // bundle__option
        $('.bundle__option input[type=radio]').change(function() {
            if ($(this).val() !== '') {
                $(this)
                    .closest('.bundle__option')
                    .addClass('on')
            }
        })
    })
